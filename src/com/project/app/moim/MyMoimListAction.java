...
public class MyMoimListAction implements Action {
@Override
	public ActionForward execute(HttpServletRequest req, HttpServletResponse resp) throws Exception {
		...
		FireMoimDTO fire = mdao.getBanUser(userid);
		String temp = req.getParameter("page"); //페이징 처리
...
//		전체 게시글의 개수
		int totalCnt = mdao.getMyMoim1Cnt(userid);
...
//		그때는 endPage를 가장 마지막 페이지 번호로 바꿔준다.
		...
		req.setAttribute("myMoimList1" , mdao.getMyMoimList1(startRow,endRow,userid));
		req.setAttribute("fire", fire);
		
		forward.setRedirect(false);
		forward.setPath("/app/moim/mymoim.jsp");
