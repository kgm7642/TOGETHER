						...
						<c:choose>
							<c:when test="${AdMoimList.size()>0 and AdMoimList != null }">
								<c:forEach var="board" items="${AdMoimList }">
									<tr class="board" id="board${board.recruitnum}" onClick="location.href='${cp}/moim/AdMoimView.mo?boardnum=${board.recruitnum}&page=${page}&keyword=${keyword}'" onmouseover="this.style.background='#eeeeeb'" onmouseout="this.style.background='white'">
					...
					<table class="pagination" style="margin: 0px;">
						<tr align="center" valign="middle">
							<td style="background-color: #f1edf7"><c:if test="${page>1}">
									<a
										href="${cp}/moim/AdMoim.mo?page=${page-1}&keyword=${keyword}">&lt;</a>
									...
									<c:choose>
										...
										<c:otherwise>
											<a href="${cp}/moim/AdMoim.mo?page=${i}&keyword=${keyword}">${i}</a>
										</c:otherwise>
									</c:choose>
...
<script>
	...
	function writemoim() {
		...
		setTimeout(
				function() {
					if (checkKingId.value == 1) {
						location.href = "${pageContext.request.contextPath}/moim/AdMoimWrite.mo?page="
								+ page + "&userid=" + userid;
					} else {
						alert("모임장이 아니면 글을 쓸 수 없습니다.");
					}
				}, 500);
	}
	...
	function checkKing() {
		...
		xhr.open("GET", cp + "/moim/CheckKingWrite.mo?userid=" + userid, true);
		xhr.send();
